
import { Button } from "@/components/ui/button";
import { ArrowRight, Calendar, Users, Shield } from "lucide-react";

const CTASection = () => {
  return (
    <section className="py-20 md:py-28 px-6 md:px-8 lg:px-12">
      <div className="container max-w-7xl mx-auto">
        <div className="relative overflow-hidden rounded-2xl">
          <div className="absolute inset-0 bg-gradient-to-r from-opsynth-600 to-accent1-600 -z-10"></div>
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] bg-[length:24px] opacity-10 -z-10"></div>
          <div className="relative px-8 py-16 md:p-16">
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-12">
                <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
                  Ready to Transform Your Infrastructure Operations?
                </h2>
                <p className="text-xl text-white/90 mb-8 leading-relaxed">
                  Join enterprise infrastructure teams who are reducing operational toil and accelerating response times with AI-powered infrastructure control.
                </p>
              </div>

              {/* Key Benefits */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="text-center">
                  <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-full mb-4">
                    <Calendar className="h-8 w-8 text-white" />
                  </div>
                  <h3 className="text-lg font-semibold text-white mb-2">Early Access</h3>
                  <p className="text-white/80 text-sm">
                    Get priority access to OpSynth before the general availability launch in Q4 2025.
                  </p>
                </div>
                <div className="text-center">
                  <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-full mb-4">
                    <Users className="h-8 w-8 text-white" />
                  </div>
                  <h3 className="text-lg font-semibold text-white mb-2">Pilot Program</h3>
                  <p className="text-white/80 text-sm">
                    Participate in our pilot program to shape the product and receive personalized onboarding support.
                  </p>
                </div>
                <div className="text-center">
                  <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-full mb-4">
                    <Shield className="h-8 w-8 text-white" />
                  </div>
                  <h3 className="text-lg font-semibold text-white mb-2">Enterprise Ready</h3>
                  <p className="text-white/80 text-sm">
                    Built for enterprise security requirements with SOC2, HIPAA, and FedRAMP compliance.
                  </p>
                </div>
              </div>

              {/* CTA Buttons */}
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                <Button size="lg" className="bg-white text-opsynth-600 hover:bg-white/90 text-lg px-8 py-4 font-semibold">
                  <a href="https://forms.gle/KenQoq3VwLtZhSxk7" target="_blank" rel="noopener noreferrer" className="flex items-center">
                    Join Interest List <ArrowRight size={20} className="ml-2" />
                  </a>
                </Button>
                <Button size="lg" variant="outline" className="border-white text-white hover:bg-white/10 px-8 py-4 text-lg">
                  <a href="/app" className="flex items-center">
                    View Live Demo
                  </a>
                </Button>
              </div>

              {/* Launch Timeline */}
              <div className="mt-12 text-center">
                <p className="text-white/80 text-sm mb-2">Target Launch Timeline</p>
                <div className="flex justify-center items-center gap-8 text-white/90">
                  <div className="text-center">
                    <div className="font-semibold">Aug 2025</div>
                    <div className="text-xs text-white/70">MVP Launch @ VMware Explore</div>
                  </div>
                  <div className="w-8 h-px bg-white/30"></div>
                  <div className="text-center">
                    <div className="font-semibold">Q4 2025</div>
                    <div className="text-xs text-white/70">General Availability</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default CTASection;
